...
def ssh_main(index_start, index_end):
    ...
    output = remote_session.get_cmd_output("ls /cs/labs/Academic/dataset/ILSVRC2012/ILSVRC2012_test/")
    image_names = output.split(" ")
    image_names = [img for img in image_names if img != ""]
    for image_name in image_names[index_start, index_end]:
        remote_session.get(os.path.join(IMAGES_TEST_PATH, image_name),
